...
module.exports = (srv)  => {
  ...
  // Reduce stock of ordered books if available stock suffices
  srv.on ('UPDATE','submitOrder', async req => {
    ...
    const newStock = currentStock - order[0].amount
    console.log(data , ID)
    if (newStock < 0) {
      req.error(403, `Not enough stock for book ${order[0].book_ID}. Current stock: ${currentStock}, requested: ${order[0].amount}`);
    }
    ...
    const result = await trx.run(UPDATE('Orders').set(data).where({ ID }));
    if (result === 0) req.reject(404, `Order with ID ${ID} not found`);
    return { data, message: `Order ${ID} submitted successfully` };
  });
